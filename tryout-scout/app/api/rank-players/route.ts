...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: deleteError } = await supabase.from("rankings").delete().eq("session_id", sessionId);
    if (deleteError) {
      return NextResponse.json({ error: "Failed to refresh rankings." }, { status: 500 });
    }

    const { error: insertError } = await supabase.from("rankings").insert(rankings);
    if (insertError) {
    ...
    console.error("Ranking generation failed:", err);
    return NextResponse.json({ error: "Something went wrong generating rankings." }, { status: 500 });
  }
}
